...
export default function AccountSubscription() {
  ...
  const subscriptionData = useQuery(api.users.getSubscriptionAndUsage);

  const notAuthenticated = !session;
  ...
  const remainingCredits = subscriptionData?.remainingCredits || 20;
  const remainingSearches = subscriptionData?.remainingSearches || 10;
  const remainingResearches = subscriptionData?.remainingResearches || 5;
        ...
        <div className="space-y-4">
          <div className="flex flex-col gap-4 bg-card p-4 rounded-lg border">
            ...
            <p className="text-sm text-muted-foreground">
              The essential features of Aether AI.
            </p>
          </div>
              ...
              <div
                ...
                style={{
                  width: limits?.CREDITS
                    ? `${Math.min(
                        100,
                        ((limits.CREDITS - remainingCredits) / limits.CREDITS) *
                          100
                      )}%`
                    : "0%",
                }}
              ...
              <div
                ...
                style={{
                  width: limits?.SEARCH
                    ? `${Math.min(
                        100,
                        ((limits.SEARCH - remainingSearches) / limits.SEARCH) *
                          100
                      )}%`
                    : "0%",
                }}
